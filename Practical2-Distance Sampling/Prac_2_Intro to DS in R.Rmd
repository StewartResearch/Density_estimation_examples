...
::: {.alert .alert-info}
<strong>Careful!</strong> A guaranteed way to produce incorrect results from your analysis is to misspecify the units distances are measured. The `ds` function has an argument `convert_units` where the user provides a value to report density in proper units. Providing an incorrect value will result in estimates that are out by orders of magnitude.
:::
